...
{
    string t_name;
    struct Student s_array[5]; //这里要对学生结构体数组初始化为5，否则报错
};
...
void structspace(struct Teacher t_array[], int t_len, int s_len)
{
    for (int i = 0; i < t_len; i++)
    {
        for (int j = 0; j < s_len; j++)
        {
            ...
            t_array[i].t_name = "Teacher";
            t_array[i].t_name += name_seed[i];

            t_array[i].s_array[j].s_name = "Student";
    ...
    for (int i = 0; i < t_len; i++) //外层for循环，遍历并打印教师信息
    {
        ...
        for (int j = 0; j < s_len; j++) //内存for循环，遍历并打印学生信息
        {
            cout << "\t 学生名字: " << t_array[i].s_array[j].s_name
                 << "\t 学生成绩: " << t_array[i].s_array[j].score;
        }
        cout << endl;
    }
}
...
int main()
{
    ...
    //经反复测试，srand函数要放在main函数里面，否则不起作用
